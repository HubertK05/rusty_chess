...
use crate::{move_register::models::{ChessMove, RawMoveType, PromotedPieceType, CastleType, MoveType}, board_setup::models::{FenNotation, Board, BoardError}, move_generator::models::{Moves, Square, PieceType, Color}};
...
#[cfg(test)]
mod tests {
    use crate::{board_setup::models::FenNotation, move_register::models::{ChessMove, MoveType, PromotedPieceType, CastleType}, move_generator::models::{Square, PieceType}};
    ...
    fn piece_move_with_file_test() {
        let res = parse_move(
            FenNotation("r4rk1/pppq1ppp/2npbn2/2b1p3/2B1P3/2NPBN2/PPPQ1PPP/R4RK1 w - - 10 9".into()),
            "Rad1".to_string()
        );
        assert_eq!(
            res.unwrap(),
    ...
    fn castle_test() {
        let res = parse_move(
            FenNotation("r1bqk1nr/pppp1ppp/2n5/2b1p3/2B1P3/5N2/PPPP1PPP/RNBQK2R w KQkq - 4 4".into()),
            "O-O".to_string()
        );
        assert_eq!(
            res.unwrap(),
