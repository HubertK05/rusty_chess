...
use backend::{
    board_setup::models::{Board, FenNotation}, move_generator::{models::{Moves, Color, PieceType, Square}}, move_register::models::{MoveType, ChessMove}, chess_bot::{search_game_tree, search_game_tree_base_case, get_ordered_moves, evaluate_chg, is_endgame, is_attacked_by_pawn}, opening_book::get_opening_book,
};
use easybench::bench;
...
async fn main() {
    ...
    let book = get_opening_book().await.unwrap();
    println!("{:#?}", book);
}
