...
fn test_count_moves(board: &Board, depth: u8, max_depth: u8) -> (u64, u64, u64) {
    ...
    move_set.0.into_iter().map(|test_move| {
        ...
        let res = test_move.register_move(&mut new_board);
        if res.is_err() {
            println!("{:?}\n{}\n{:?}", res, board, test_move);
            panic!("wtf is this?");
        }
        new_board.turn = new_board.turn.opp();
...
#[test]
fn position_3() {
    let board = Board::try_from(FenNotation("8/2p5/3p4/KP5r/1R3p1k/8/4P1P1/8 w - - 0 1".into())).unwrap();
...
#[test]
fn position_4() {
    let board = Board::try_from(FenNotation("r3k2r/Pppp1ppp/1b3nbN/nP6/BBP1P3/q4N2/Pp1P2PP/R2Q1RK1 w kq - 0 1".into())).unwrap();
...
#[test]
fn position_5() {
    let board = Board::try_from(FenNotation("rnbq1k1r/pp1Pbppp/2p5/8/2B5/8/PPP1NnPP/RNBQK2R w KQ - 1 8".into())).unwrap();
...
#[test]
fn position_6() {
    let board = Board::try_from(FenNotation("r4rk1/1pp1qppp/p1np1n2/2b1p1B1/2B1P1b1/P1NP1N2/1PP1QPPP/R4RK1 w - - 0 10".into())).unwrap();
